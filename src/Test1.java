...
public class Test1 {
	...
	private static int solution(int[] a) {
		...
		return length;
		/*
		 * LinkedList<Node> list = new LinkedList<>(); for (int i = 0; i < A.length;
		 * i++) { Node node = new Node(A[i], false); list.add(node); }
		 * 
		 * boolean isVisitedNode = false; int start = A[0]; int value = 0; while
		 * (!isVisitedNode) { Node n = list.get(start); if (!n.visited) { n.visited =
